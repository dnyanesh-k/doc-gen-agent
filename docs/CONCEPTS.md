...
The doc-gen-agent follows a 6-stage pipeline from code changes to generated documentation.
...
**Components**:
- **Change Detector**: Validates git repo, gets remote branch
- **Change Analyzer**: Gets changed files, diffs, file content, and stats

**Output**: List of changed files with diff, content, and change type

**Files**: `src/ingestion/change_detector.py`, `src/ingestion/change_analyzer.py`

**Note**: Git's `.gitignore` handles file filtering - no separate preprocessing needed.
...
**Components**:
- **Tokenizer**: Counts tokens using tiktoken (manages context window limits)
- **Chunker**: Splits large files into smaller chunks (line-based with overlap)
- **Embedding Generator**: Converts chunks to vectors using sentence-transformers

**Output**: Token counts, chunks, and embedding vectors

**Files**: `src/feature_extraction/tokenizer.py`, `src/feature_extraction/chunker.py`, `src/feature_extraction/embeddings.py`

**Model Used**: `all-MiniLM-L6-v2` (384 dimensions, free, local)
...
- **Semantic Index**: Index structure for fast similarity queries
...
**Components**:
- **Similarity Search**: Find similar code chunks using cosine similarity
...
**Components**:
- **Context Builder**: Assembles retrieved context and change summary
- **Prompt Templates**: Templates for different doc types
- **LLM**: Generates documentation from prompts
...
**Output**: Documentation files saved locally
...
Code Changes
    ↓
[1. Ingestion] → Changed Files + Diff + Content
    ↓
[2. Feature Extraction] → Token Counts + Chunks + Embeddings
    ↓
[3. Indexing] → Stored in Vector DB
    ↓
[4. Retrieval] → Similar Code Context
    ↓
[5. Generation] → Documentation (Markdown)
    ↓
[6. Post-Processing] → Saved Files
```
...
|-------|--------|-------------|
| 1. Ingestion | ✅ Done | Git change detection, diff, content |
| 2. Feature Extraction | ✅ Done | Tokenizer, chunker, embeddings |
| 3. Indexing | ⏳ Next | PostgreSQL + pgvector |
| 4. Retrieval | ⏳ Pending | Semantic search |
| 5. Generation | ⏳ Pending | LLM integration |
| 6. Post-Processing | ⏳ Pending | Save files |